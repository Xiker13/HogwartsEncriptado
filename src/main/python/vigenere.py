...
def validar_datos(texto: str, clave: str) -> Tuple[bool, str]:
    ...
    if clave_normalizada != clave.upper():
       print(
           f"AVISO: Se han eliminado caracteres no validos de la clave. "
...
def leer_fichero(path: str) -> str:
    ...
    try:
        ...
        print(f"[AVISO] {path} no está en UTF-8, probando Latin-1...")
        try:
...
def descifrar_vigenere(texto_cifrado: str, clave: str) -> str:
    ...
    if not any(ch in string.ascii_uppercase for ch in texto_descifrado):
        print("⚠️ Posible clave incorrecta: el resultado no contiene letras legibles.")
...
def descifrar_archivo(ruta_entrada: str, ruta_salida: str, clave: str) -> None:
    ...
    """
    if not os.path.exists(ruta_entrada):
        print("[ERROR] El archivo cifrado no existe.")
        return

    contenido = leer_fichero(ruta_entrada)
    if not contenido:
        print("[ERROR] No se pudo leer el archivo cifrado.")
        return
...
def demo() -> None:
    ...
    print("=== DEMO VIGENÈRE ===")
    print(f"Texto demo : {TEXTO_DEMO}")
    print(f"Clave demo : {CLAVE_DEMO}")
    ...
    if len(args) == 1:
        ...
        try:
            if modo == "cifrar" and len(args) >= 4:
                ...
                # Uso incorrecto
                print("Uso:")
                print('  python vigenere.py cifrar "TEXTO" "CLAVE"')
                print('  python vigenere.py descifrar "TEXTO" "CLAVE"')
                print("  python vigenere.py cifrar-archivo entrada.txt salida.txt CLAVE")
                print("  python vigenere.py descifrar-archivo entrada.txt salida.txt CLAVE")
                sys.exit(1)
        except ValueError as e:
            # Errores de validación (clave mala, texto vacío, etc.)
            print(str(e))
            sys.exit(1)
